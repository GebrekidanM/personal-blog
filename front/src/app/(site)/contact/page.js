...
export default function ContactPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://formspree.io/f/xblqroez', {
        method: 'POST',
            ...
            <div>
              <button type="submit" disabled={loading}
                ...
                className={`w-full font-bold py-3 px-6 rounded-lg transition duration-300 ${
                  loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-gray-800 hover:bg-gray-700 text-white'
                }`}>
                {loading ? 'Sending...' : 'Send Message'}
              </button>
            </div>
            {status && <p className={`text-center mt-2 ${status.startsWith('✅') ? 'text-green-600' : 'text-red-600'}`}>{status}</p>}
          </form>
